...
# Constants matching training
IMAGE_SIZE = 256  # Make sure this matches your model's expected input size
CLASS_NAMES = ["oily", "normal", "dry"]

# Load model (Ensure you have the correct model format: .h5 for Keras models)
try:
    MODEL = load_model(r"FYP\Training\vit_model_bs4_lr0.1_Adam.h5")  # Update to .h5 if it's a Keras model
    logger.info("Model loaded successfully.")
except Exception as e:
    logger.error(f"Failed to load model: {str(e)}")
    raise RuntimeError("Model loading failed")
...
def read_file_as_image(data) -> np.ndarray:
    """
    Read and convert image data to a numpy array.
    """
    try:
        image = Image.open(BytesIO(data))
        if image.mode != 'RGB':
